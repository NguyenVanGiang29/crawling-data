...
def getContentComment(driver):
    try:
        ...
        if (len(links)):
            for link in links:
                takeLink = link.get_attribute('href').split('ctoken=')[
                    1].split('&')[0]
...
def getnumOfPostGroup(driver, pageId, amount, filePath):
    driver.get("https://touch.facebook.com/" + pageId)
    while len(readData(filePath)) < amount:
        getPostIds(driver, filePath)


def getUserIds(driver, filePath='users.csv'):
    allUsers = readData(filePath)
    sleep(2)
    driver.execute_script("window.scrollTo(0, document.body.scrollHeight)")
    shareBtn = driver.find_elements_by_xpath(
        '//a[contains(@href, "/groups/919166571930352/user/")]')
    if (len(shareBtn)):
        for link in shareBtn:
            userId = link.get_attribute('href')
            if (userId) not in allUsers:
                print(userId)
                writeFileTxt(filePath, userId)


# https://touch.facebook.com/ - https://www.facebook.com/
def getnumOfUserGroup(driver, pageId, amount, filePath='users.csv'):
    driver.get("https://www.facebook.com/" + pageId)
    while len(readData(filePath)) < amount:
        getUserIds(driver, filePath)

cookie = 'c_user=100055482456734;xs=28%3A-Cy81quc61X5tw%3A2%3A1645419106%3A-1%3A16295'
driver = initDriver()
isLive = checkLiveCookie(driver, cookie)
if (isLive):
    #Get PostId from group and comment of post
    getnumOfPostGroup(driver, 'groups/919166571930352', 1000, 'posts.csv')
    for postId in readData('posts.csv'):
        getAmountOfComments(driver, postId, 1000)
    #Get UserId from member of group
    getnumOfUserGroup(
        driver, 'groups/919166571930352/members/things_in_common', 2000, 'users.csv')
